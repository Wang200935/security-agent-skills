...
- Do error responses in production return stack traces, internal paths, or SQL errors?
...
- `delegate_task` — launch parallel hunting agents
- `search_files` — find code patterns, endpoints, sinks
- `read_file` — read specific files at depth
- `terminal` — run grep, ripgrep for code patterns
- `write_file` — record findings for Phase 3