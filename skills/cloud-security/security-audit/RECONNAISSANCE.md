...
Before looking for bugs, understand what you're auditing. This requires depth, not just a directory listing. Launch **multiple `research` agents in parallel** via `delegate_task` to map different aspects of the codebase:
...
- `delegate_task` — launch parallel research agents (use `role: "leaf"`, `toolsets: ["file", "search", "terminal"]`)
- `search_files` — find files by pattern, search content
- `read_file` — read specific files with line numbers
- `write_file` — write `architecture.md` to output directory