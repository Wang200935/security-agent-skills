...
- **`delegate_task` tool** means Hermes's delegation/sub-agent mechanism (use `delegate_task` with `role: leaf`).
- **`research` agent** means a delegated agent optimized for focused codebase exploration and factual verification (use `delegate_task` with `role: leaf` and focused context).
- **`general` agent** means a delegated agent that can investigate broadly and spawn focused research agents (use `delegate_task` with `role: leaf` and broader context).
- **`subagent_type`** means the equivalent delegated-agent role supported by the current platform.

Use Hermes's native capabilities while preserving the specified roles, parallelism, prompts, and independence boundaries.
...
Subagents (Phases 2, 3, 6) do NOT write files — they return results to you via `delegate_task`. You are responsible for writing all files to the output directory using `write_file`.
...
2. **Hunt** — Use `HUNTING.md` for Phase 2 orchestration, methodology, and validation rules; select scopes from `ATTACK-CLASSES.md`.
...
- User mentions security, vulnerabilities, penetration testing, audit, or red-teaming in context of a codebase
- Task involves finding bugs in application code with security impact
...
trivy fs --scanners vuln,config,secret,license --format json --output trivy-report.json .

# CI-friendly table output with exit code on HIGH/CRITICAL
trivy fs --scanners vuln --severity HIGH,CRITICAL --exit-code 1 .

# Generate dependency report for compliance
trivy fs --format template --template "@contrib/sarif.tpl" --output trivy.sarif .
```

**Integration**: Run in `security-audit` Phase 1 pre-flight; feed findings to hunting agents for supply-chain attack chain building.
...
```bash
# 1. Semgrep fast triage (OWASP ruleset + custom)
semgrep scan --config=p/owasp-top-ten --config=p/secrets --config=p/ci --json=semgrep.json .

# 2. Derive targeted AST-grep paths from semgrep scope
python3 -c "
import json, sys
...
for r in d.get('results', []):
    paths.add(r['path'])
open('sg-targets.txt','w').write('\n'.join(sorted(paths)))
"

# 3. AST-grep structural pass on scoped paths
xargs -a sg-targets.txt -I{} sg run -p 'pattern' {}

# 4. Secrets (working tree + history)
gitleaks detect --source . --report-format json --report-path gitleaks.json
trufflehog filesystem . --json > trufflehog.json

# 5. Trivy fs (misconfig + deps + secrets + licenses)
trivy fs --scanners config,vuln,secret,license --format json --output trivy-fs.json .

# 6. Triage -> prioritize dynamic validation
#    Cross-reference semgrep HIGH + trivy CRITICAL + gitleaks hits -> those paths first
```

**Output**: Ranked target list for dynamic validation agents. Integrate into Phase 1 reconnaissance.
