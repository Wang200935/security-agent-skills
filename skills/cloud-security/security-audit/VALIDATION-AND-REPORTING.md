...
For every finding that survived Phase 3 validation, produce a structured JSON object conforming to the schema defined in `report-schema.json` (in the same directory as this skill file — read it via `read_file` before writing output). Write the result to `<output-dir>/findings.json`.
...
Launch **one `research` agent per confirmed finding** via `delegate_task`, all in parallel. Each agent gets exactly one finding from `findings.json` and verifies it independently. Give each agent the JSON object for its finding and this prompt:
...
**Tools to use in these phases:**
- `delegate_task` — launch validation agents (Phase 3) and verification agents (Phase 6)
- `read_file` — read source code for verification, read `report-schema.json`
- `write_file` — write `REPORT.md`, `FINDINGS-DETAIL.md`, `findings.json`
- `terminal` — run `node validate-findings.cjs findings.json` for schema validation
- `search_files` — verify code paths, endpoints, and line numbers