---
name: rf-clown-master
description: RF-Clown v2 完整移植技術手冊 — 從 RF-Clown 3×nRF24 原始碼深度分析、nyanBOX sigkill 交叉比較、到 Wang 現有雙 nRF24 硬體的移植方案。包含完整硬體腳位對照、firmware 逐行架構分析、UI 動畫系統、頻道協議映射、編譯燒錄 SOP、以及所有已踩過的 Pitfall。當 user 要製作 OLED 選單多協議干擾器、或移植 RF-Clown/nyanBOX 到 Wang 硬體時載入。
version: 1.0.0
author: Hermes Agent (2026-07-14 deep research)
license: MIT
metadata:
  hermes:
    tags: [rf-clown, nyanbox, esp32, nrf24, jammer, multi-protocol, oled,移植]
    related_skills: [esp32-nrf24-jammer-builder, ctf-general]
---

# RF-Clown v2 完整移植技術手冊

## 🎯 觸發時機

- user 提到「RF-Clown」「nyanBOX」「多協議干擾器」「OLED 選單 jammer」
- user 要移植 RF-Clown 到 Wang 現有硬體（雙 nRF24 + OLED + 按鈕）
- user 問 RF-Clown 內部架構、UI 動畫、頻道定義、nRF24 初始化策略

---

## 📊 RF-Clown v2 vs nyanBOX 深度對比

| 特徵 | RF-Clown v2 | nyanBOX |
|------|-----------|---------|
| GitHub | cifertech/RF-Clown ★1,721 | jbohack/nyanBOX ★547 |
...
## 🏗 RF-Clown 完整 firmware 架構逐行分析

### 檔案結構

```
RfClown/
├── RfClown.ino     ← 主程式 (407 行)
...
```
┌──────────────────────────┐ y=0
│ ████████ RF-Clown v2.0.0 │ HEADER_H=12 (黑底白字)
├──────────────────────────┤ y=12
│                          │
│  ┌──────────────────┐    │ y=16 (HEADER_H + 4)
...
### Wang 現有硬體 vs RF-Clown 差異

| 項目 | RF-Clown | Wang 現有 | 移植動作 |
|------|----------|----------|---------|
...
RF-Clown 用 ESP32 core **1.0.5**。Wang 環境用 **3.3.10**。
...
RF-Clown 寫的時候可能是 RF24 1.4.x（`startConstCarrier` 回 `bool`）。
Wang 環境是 RF24 1.6.1（回 `void`）。

**移植時不需要改** — RF-Clown 原始碼的 `configure_Radio()` 沒有檢查回傳值（直接 `radio.startConstCarrier(...)`），所以相容 ✅。但如果自己加的 code 有 `if (!radio.startConstCarrier(...))` 就會編譯失敗。
...
`rfclown_multi.ino` 必須在 `rfclown_multi/rfclown_multi.ino`
...
RF-Clown 用 `pinMode(PIN_BTN_L, INPUT_PULLUP)` — ESP32 內建上拉，不用外部電阻。
...
RF-Clown 用 3 顆 nRF24 共用 default SPI（VSPI 硬體腳）。Wang 硬體目前用 **VSPI+HSPI 獨立**。
移植時兩種做法都可以：
...
RF-Clown 用三顆 radio 各有固定頻道群組。移植到兩顆時：
- **方案 A**：radioA 鎖廣告通道 (2/26/80)，radioB 做 RF-KILL 混亂掃描 (全 0-79)
- **方案 B**：雙 radio 同步隨機跳頻，像 nyanBOX sigkill 做法（每模式各有頻道陣列）
- **方案 C**：照 RF-Clown 原設計，砍掉 RadioC，RadioA/B 各自鎖一半頻道群組

推薦 **方案 B** (nyanBOX 風格) — 最簡單、最符合 user 描述的多協議選單行為。
...
arduino-cli compile \
  ...
  --output-dir bin \
  rfclown_multi/rfclown_multi.ino
```
...
~/.hermes/hermes-agent/venv/bin/python3 -m esptool \
  ...
  write-flash 0x1000 bin/rfclown_multi.ino.bootloader.bin

# partitions + app
~/.hermes/hermes-agent/venv/bin/python3 -m esptool \
  --chip esp32 --port $ESPPORT --baud 230400 \
  --before default-reset --after hard-reset \
  write-flash 0x8000 bin/rfclown_multi.ino.partitions.bin \
  0x10000 bin/rfclown_multi.ino.bin
```
...
- RF-Clown 完整 clone: `~/projects/rf-clown-study/`
...
- cifertech/RF-Clown (GitHub, ★1,721): https://github.com/cifertech/RF-Clown
- jbohack/nyanBOX (GitHub, ★547): https://github.com/jbohack/nyanBOX
- RF-Clown 官方文件: https://cifertech.net/rf-clown-your-portable-ble-bluetooth-jamming-tool/
- nyanBOX 商店: https://nyandevices.com