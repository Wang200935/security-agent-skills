...
tags: [OSINT, username-scanner, social-media, reconnaissance, account-discovery, aliens-eye]
related_skills:
  - osint
  - cybersecurity
  - pentest
...
**Package**: `aliens-eye` v2.1.0 (PyPI)  
**Binary**: `aliens_eye` (installed in Hermes venv)  
**Python path**: `~/.hermes/hermes-agent/venv/bin/aliens_eye`  
...
| ML retrain | `aliens_eye train collect` → `aliens_eye train fit` |
| Self-check | `aliens_eye selfcheck` validates detection accuracy |
...
aliens_eye username --profile quick      # fast, fewer sites (~30s via Tor, ~20s direct)
aliens_eye username --profile full       # all sites, default settings
aliens_eye username --profile aggressive # more retries, slower but thorough

# ── Target specific platforms ──
aliens_eye username --site github,reddit,gitlab

# ── Exclude platforms ──
aliens_eye username --exclude-site pornhub,onlyfans
...
aliens_eye target_user --profile full --format json --output ./osint_results
```

### Step 2: Enrich with Advanced Variations
```bash
# Check username variations (common patterns)
aliens_eye target_user -l advanced --format all --output ./osint_results_advanced
```

### Step 3: Combine with Other OSINT Tools
```bash
# Parallel workflow:
# Terminal 1: aliens_eye for social media footprint
aliens_eye target_user --profile full --format all --output ./osint/
...
aliens_eye target_user --profile full --format html --output ./osint_report/

# Markdown for documentation
aliens_eye target_user --profile full --format md --output ./osint_report/
```
...
1. `./config.json` (cwd)
2. `~/.config/aliens_eye/config.json` (Linux/macOS)
3. `%LOCALAPPDATA%\aliens_eye` (Windows)
...
```bash
~/.hermes/hermes-agent/venv/bin/aliens_eye username --profile full --format all --output /tmp/aliens_eye_results
```
...
|:-----|:----------|:-------------|:------|:-------|:------|
| **Aliens Eye** | 840+ | ✅ ML+heuristic | Fast (async) | JSON/CSV/HTML/MD | Best detection quality |
...
- Tools like Sherlock/Aliens Eye are ASCII-only

**Parallel 4-leg workflow:**

| Leg | Tool | What it finds |
|:---|:---|:---|
| 1. English username variants via Aliens Eye | `aliens_eye` over Tor | Catches ASCII accounts (rare for Chinese names but worth 5 min) |
| 2. Pinyin variants | `aliens_eye hongzhong zhaohongzhong hzzhao ...` | Same as leg 1 with different romanization |
...
# Run Aliens Eye with --no-nsfw --profile quick for each (~30s each via Tor)
...
```bash
# Track 1: Aliens Eye via Tor (background)
~/.hermes/hermes-agent/venv/bin/aliens_eye <username> --no-nsfw --tor --profile quick \
    --format json --output /tmp/osint/aliens_eye/ &

# Track 2: Aliens Eye via Tor with variations (background)
for v in variant1 variant2 variant3; do
    ~/.hermes/hermes-agent/venv/bin/aliens_eye "$v" --no-nsfw --tor --profile quick \
        --format json --output /tmp/osint/aliens_eye/ &
done
...
Tor must be running BEFORE `aliens_eye --tor` will work:
...
- **Python path**: Hermes venv Python ≠ system Python. Always use `~/.hermes/hermes-agent/venv/bin/aliens_eye`.
...
- **Playwright extra**: `--playwright` requires `pip install "aliens-eye[browser]"` + `python -m playwright install chromium`.
