---
name: email-osint
description: "Email-first OSINT investigation — 7-track parallel workflow when the seed is an email address, not a username or domain. Uses holehe (service registration), aliens-eye (username sweep), HudsonRock (infostealer breach correlation — highest value), Gravatar, GitHub commit search, direct platform checks (Instagram GraphQL, Pinterest JSON-in-HTML), and multi-engine web search. Produces identity resolution with confidence tiers."
version: 1.0.0
...
Seed: <email>@<domain>
  ...
  ├── Track 2: aliens-eye ── local-part as username sweep across 840+ sites
...
### Track 2: aliens-eye (username sweep)

```bash
~/.hermes/hermes-agent/venv/bin/aliens_eye <local-part> --no-nsfw --profile full --format json --output /tmp/osint_<name>/
```
...
Cross-variant check (same `final_url` returns Found for 2+ username variants) is the strongest real-account signal. See the `aliens-eye` skill for the full filtering recipe.
...
- **aliens-eye inflated "Found"**: Filter by `ai_analysis.probability >= 0.80` + manual verify. 128 raw Found → ~5-10 real after filtering.
...
- **aliens-eye `--profile` arg**: Only `quick`/`full`/`aggressive`. Not `basic`/`standard`/`intermediate`.
