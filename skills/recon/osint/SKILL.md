...
# Platform checkers
USERNAME_TOOLS = [
    'aliens_eye',       # 840+ platforms — ML+heuristic detection (BEST quality) — pip install aliens-eye
...
# ── Aliens Eye (preferred for quality) ──
# Full scan with ML detection + all export formats:
#   aliens_eye username --profile full --format all --output results/
# Advanced level checks prefix/suffix variations:
#   aliens_eye username -l advanced --format json
# Through Tor for OPSEC:
#   aliens_eye username --tor --profile full
# Target specific platforms:
#   aliens_eye username --site github,reddit,gitlab
# Run alongside others for maximum coverage:
#   aliens_eye username --profile full & sherlock username & maigret username --all-sites
...
OSINT_TOOLS_2025_2026 = {
    ...
    'username_scanning': {
        'tools': ['aliens_eye (840+ platforms, ML detection)', 'Sherlock (300+)', 'Maigret (2500+)', 'WhatsMyName (500+)'],
        'note': 'aliens_eye supports Tor, Playwright fallback, ML-blended detection. Cross-variant username scanning is strongest signal.',
    },
    'ai_osint': {
...
**Key insight**: `yuanhan` is a common romanization for 袁瀚 / 源瀚 / 苑涵 / 遠涵 / 元瀚… — **at least 4-5 different people** actively use this handle. The 110 "high-confidence" accounts from aliens-eye are a **collage of multiple identities**, not one person.
...
5. **In reports, always separate "seed-attributed" vs "username-candidate" findings**
   ...
   - Username-candidate: 110 aliens-eye accounts (flag as "requires disambiguation")
