...
- aliens-eye (840+ platforms username check via email local-part)
...
# Aliens-eye + Maigret cross-platform username scan (if email local-part available)
aliens_eye "$USERNAME" --profile full --format json --output ~/osint-reports/ae/ &
...
- **Username guessing from email local-part is noisy**: `john.doe@example.com` → "john.doe", "johndoe", "jdoe" — these are guesses, not confirmations. Mark as "candidate username" until aliens-eye/maigret/sherlock confirm 200 status.
...
```bash
# 1. aliens-eye + Maigret + Sherlock (parallel) for breadth
aliens_eye "username" --profile full --format json -o ~/osint/ae/
...
| Username scanning quality (840+ platforms, ML detection) | `aliens-eye` skill |
