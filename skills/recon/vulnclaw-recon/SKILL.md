---
name: vulnclaw-recon
description: "信息收集流程 — 被动+主动侦察。Use when performing authorized penetration testing, CTF, or security assessment tasks related to recon."
version: 1.0.0
...
metadata:
  hermes:
    ...
    related_skills: [vulnclaw-pentest-flow, vulnclaw-web-security-advanced, vulnclaw-osint-recon]
  upstream:
    repo: https://github.com/Unclecheng-li/VulnClaw
    source_path: vulnclaw/skills/core/recon.md
    original_name: recon
---
...
- **搜尋端點分頁繞過**：當搜尋/列表端點有結果上限但支援部分匹配時，使用粒度查詢（如學校+班級組合、姓氏枚舉）繞過分頁限制，取得完整資料集。詳見 `vulnclaw-web-security-advanced` 的 `references/pagination-bypass-granular-search.md`。
...
- 上一次滲透任務中，先載入 `vulnclaw-pentest-flow` 做總路由，再依場景載入本系列專項 skill（例如 `vulnclaw-web-security-advanced`、`vulnclaw-osint-recon`）。
...
- `vulnclaw-web-security-advanced` 的 `references/pagination-bypass-granular-search.md`
- `vulnclaw-web-security-advanced` 的 `references/php-tutoring-platform-pattern.md` (feifan-wh 模式)
- `vulnclaw-web-security-advanced` 的 `references/taiwan-edutech-platform-pattern.md` (Jurui 模式)
- `vulnclaw-recon` 的 `references/feifan-md-platform-pattern.md` (非凡教育-民德 feifan-md.com.tw 模式)
- `vulnclaw-recon` 的 `references/php-tutoring-platform-pattern.md` (PHP procedural 補教平台通用模式)
